...
import { signoutLink } from '../utils/links';
...
const Navbar = () => {
  ...
  const handleLogout = async () => {
    try {
      setLoggingOut(true);
      await axios.delete(signoutLink, { withCredentials: true });
      setUser(null);
    } catch (error) {
    ...
    <nav className='z-50 h-16 w-full bg-gray-800 text-white flex items-center justify-between px-4'>
      ...
      <div>
        {user ? (
          ...
          <Link to={'/signin'} className='bg-blue-500 px-3 py-1 rounded text-md font-medium text-white disabled:bg-red-300 disabled:cursor-not-allowed transition hover:bg-blue-600 cursor-pointer'>Login</Link>
        )}
      </div>
    </nav>
  )
